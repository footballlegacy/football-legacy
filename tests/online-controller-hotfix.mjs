...
check(onlineHtml.includes('../controller-ui.js?v=169-online-menu-hotfix'), 'Online shell must load the shared controller UI');
check(onlineHtml.includes('id="hostButton" type="button" data-controller-default'), 'Host must be the default controller target');
check(onlineApp.includes("const BUILD='169'"), 'Online peers must reject pre-hotfix builds');
