...
function resolveEngineSelection(requested,matchType,online=false){
  ...
  if(normalized!==FL_V2_ENGINE)return{requested:BUILD_173_ENGINE,effective:BUILD_173_ENGINE,version:QUICK_PLAY_ENGINE_VERSION,fallbackReason:null};
  if(online||mode==='online')return{requested:FL_V2_ENGINE,effective:BUILD_173_ENGINE,version:QUICK_PLAY_ENGINE_VERSION,fallbackReason:'online-authority-frozen'};
  if(!['single-player','free-kick-suite','spectator'].includes(mode))return{requested:FL_V2_ENGINE,effective:BUILD_173_ENGINE,version:QUICK_PLAY_ENGINE_VERSION,fallbackReason:`unsupported-offline-mode:${mode}`};
...
const state={mode:initialMode,engineRequested:normalizeEngineRequest(query.get('engine')),step:'teams',homeLeague:'div1',homeTeamId:'woolwich-arsenal',awayLeague:'div1',awayTeamId:'west-london-blues',management:{home:managementSide(),away:managementSide()},setup:{stadiumValue:'',matchTime:'night',weather:'clear',matchLength:Number(query.get('matchMinutes'))||6,difficulty:'ultimate',camera:preferences.camera||'broadcast',volume:70,homeKit:'home',awayKit:'away'}};
...
function engineSummary(selection){if(selection.effective===FL_V2_ENGINE)return state.mode==='free-kick-suite'?'FL V2 · Strict Set-Piece Suite':state.mode==='spectator'?'FL V2 · Strict CPU vs CPU':'FL V2 · Strict Single Player';if(selection.fallbackReason)return'Build 173 · FL V2 unavailable for this mode';return'Build 173 · Stable'}
function updateEngineSelectionUI(){
  ...
  elements.gameplayEngine.value=state.engineRequested;
  elements.gameplayEngine.disabled=ONLINE;
  elements.gameplayEngine.setAttribute('aria-disabled',String(ONLINE));
  elements.engineStatus.dataset.effectiveEngine=selection.effective;
  if(selection.effective===FL_V2_ENGINE)elements.engineStatus.innerHTML=state.mode==='free-kick-suite'?'<strong>FL V2 active · Strict Set-Piece Suite:</strong> the reviewed Clock, Restart, Set-Piece and coordinate authority will run this offline route. If V2 faults, the match stops with an exportable diagnostic; it never continues as Build 173.':state.mode==='spectator'?'<strong>FL V2 active · Strict CPU vs CPU:</strong> both teams use the deterministic Ball, Movement, CPU, Formation, First Touch and Aerial authority. If V2 faults, the match stops with an exportable diagnostic; it never continues as Build 173.':'<strong>FL V2 active · Strict Single Player:</strong> the deterministic Ball, Movement, CPU, Formation, First Touch and Aerial authority will run this offline match. If V2 faults, the match stops with an exportable diagnostic; it never continues as Build 173.';
  else if(selection.fallbackReason==='online-authority-frozen')elements.engineStatus.innerHTML='<strong>Build 173 active:</strong> Online is frozen to the proven engine. FL V2 is available only in offline Single Player, CPU vs CPU and the Set-Piece Suite.';
  else if(selection.fallbackReason)elements.engineStatus.innerHTML=`<strong>Build 173 selected before kickoff:</strong> FL V2 is not supported in ${esc(engineModeLabel(state.mode))}. Switch to Single Player, CPU vs CPU or the Set-Piece Suite to use strict V2.`;
  else elements.engineStatus.innerHTML='<strong>Build 173 active · Stable:</strong> every Quick Play workflow continues to use the existing match engine unless FL V2 is explicitly selected for offline Single Player, CPU vs CPU or the Set-Piece Suite.';
}
