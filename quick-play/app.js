...
function resolveEngineSelection(requested,matchType,online=false){
  ...
  if(!['single-player','free-kick-suite'].includes(mode))return{requested:FL_V2_ENGINE,effective:BUILD_173_ENGINE,version:QUICK_PLAY_ENGINE_VERSION,fallbackReason:`unsupported-offline-mode:${mode}`};
...
function engineModeLabel(mode){return mode==='free-kick-suite'?'Free Kick Practice':mode==='spectator'?'CPU vs CPU':mode==='co-op'?'Co-op / Local 2P':mode==='home-co-op'?'Same-Team Co-op':mode==='online'?'Online Versus':'Single Player'}
function engineSummary(selection){if(selection.effective===FL_V2_ENGINE)return state.mode==='free-kick-suite'?'FL V2 · Experimental Set-Piece Suite':'FL V2 · Experimental Single Player';if(selection.fallbackReason)return'Build 173 · FL V2 safely deferred';return'Build 173 · Stable'}
function updateEngineSelectionUI(){
  ...
  if(selection.effective===FL_V2_ENGINE)elements.engineStatus.innerHTML=state.mode==='free-kick-suite'?'<strong>FL V2 active · Experimental Set-Piece Suite:</strong> the reviewed Clock, Restart, Set-Piece and coordinate authority will run this offline practice route. Build 173 remains the immediate safety fallback.':'<strong>FL V2 active · Experimental Single Player:</strong> the new deterministic Ball, Movement, CPU, Formation, First Touch and Aerial authority will run this offline match. Build 173 remains the immediate safety fallback.';
  else if(selection.fallbackReason==='online-authority-frozen')elements.engineStatus.innerHTML='<strong>Build 173 active:</strong> Online is frozen to the proven engine. FL V2 remains an offline Single Player playtest only.';
  else if(selection.fallbackReason)elements.engineStatus.innerHTML=`<strong>Build 173 active:</strong> FL V2 remains selected for Single Player, but ${esc(engineModeLabel(state.mode))} is not yet supported. Switching back to Single Player restores FL V2.`;
  else elements.engineStatus.innerHTML='<strong>Build 173 active · Stable:</strong> every Quick Play workflow continues to use the existing match engine unless FL V2 is explicitly selected for offline Single Player or the Set-Piece Suite.';
}
