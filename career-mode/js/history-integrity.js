window.FLHistoryIntegrity=(()=>{
  ...
  function ensureManager(game,profile,club,year){
    ensure(game);
    ...
    game.managerArchive.filter(old=>old.id!==profile.id&&old.currentClubId===club.id).forEach(old=>{old.currentClubId=null;const prior=array(old.clubs).at(-1);if(prior&&prior.to==='Present')prior.to=Math.max(number(prior.from)||year-1,year-1)});
    let row=game.managerArchive.find(x=>x.id===profile.id);
    if(!row){row={...profile,id:profile.id,clubs:[],seasons:[],honours:[],achievements:[],record:{played:0,won:0,drawn:0,lost:0,gf:0,ga:0,winPct:0},firstSeason:year,lastSeason:year,currentClubId:club.id,lastClubId:club.id};game.managerArchive.push(row)}
