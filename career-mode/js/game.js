window.FLGame = (() => {
  ...
  function makePlayers(club, r, year=1888, gameRef=null){
    ...
    return FLData.positions.map((position,index)=>{
      ...
      const player={
        ...
        honours:[],seasonHistory:[],matchHistory:[]
      };
  ...
  function archiveSeason(game){
    ...
    const toRows=block=>block.table.map((team,index)=>{const base={position:index+1,id:team.id,name:team.name,played:Number(team.played)||0,won:Number(team.won)||0,drawn:Number(team.drawn)||0,lost:Number(team.lost)||0,gf:Number(team.gf)||0,ga:Number(team.ga)||0,points:Number(team.points)||0,divisionId:block.division.id,division:block.division.name,tier:block.division.tier};if(headless&&Number(block.division.tier)>5&&block.division.id!==controlledDivision?.id)return {position:base.position,id:base.id,name:base.name,points:base.points,divisionId:base.divisionId,division:base.division,tier:base.tier};return headless?base:{...base,stature:+Number(team.stature||0).toFixed(2),momentum:+Number(team.momentum||0).toFixed(2),powerRating:+Number(team.powerRating||team.clubRating||0).toFixed(2),trajectory:team.currentTrajectory||'flat',cause:team.currentCause||''}});
    ...
    activeTeams.forEach(team=>{const block=pyramidRows.find(x=>x.division.id===team.divisionId),row=block?.table.find(x=>x.id===team.id),entry={season:label,division:block?.division.name||'—',divisionId:block?.division.id||null,tier:block?.division.tier||team.tier||'—',position:row?.position||'—',points:row?.points||0};if(!headless||team.id===game.meta?.preselectedClubId)Object.assign(entry,{played:row?.played||0,won:row?.won||0,drawn:row?.drawn||0,lost:row?.lost||0,gf:row?.gf||0,ga:row?.ga||0});team.seasonHistory.push(entry)});
